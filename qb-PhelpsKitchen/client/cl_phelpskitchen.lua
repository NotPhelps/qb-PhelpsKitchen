...
CreateThread(function()
    phelpskitchen = AddBlipForCoord(82.86, 281.48, 110.21)
    SetBlipSprite (phelpskitchen, 269)
    SetBlipDisplay(phelpskitchen, 4)
    SetBlipScale  (phelpskitchen, 0.6)
    SetBlipAsShortRange(phelpskitchen, true)
    SetBlipColour(phelpskitchen, 51)
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentSubstringPlayerName("Phelps Kitchen")
    EndTextCommandSetBlipName(phelpskitchen)
end)
...
AddEventHandler('onResourceStop', function(resourceName) 
    if GetCurrentResourceName() == resourceName then
        for k, v in pairs(Config.Zones) do
            exports['qb-target']:RemoveZone("phelpskitchen"..k)
        end
        DeletePed(jobPed)
    end 
end)
...
function phelpskitchenZones()
    for k, v in pairs(Config.Zones) do
        exports['qb-target']:AddCircleZone("phelpskitchen"..k, v.coords, v.radius, {
            name = "phelpskitchen"..k,
            debugPoly = false,
            useZ=true,
        }, {
            options = {
    ...
    if not DoesEntityExist(jobPed) then
        ...
        exports['qb-target']:AddTargetEntity(jobPed, { 
            ...
            { 
                type = "client",
                event = "phelpskitchen:client:jobGarage",
            ...
            { 
                type = "client",
                event = "phelpskitchen:client:storeGarage",
                icon = "fa-solid fa-clipboard-check",
                label = "Store Vehicle",
                job = "phelpskitchens"
            },
...
RegisterNetEvent('phelpskitchen:client:jobGarage', function(vehicle)
    local vehicle = Config.Vehicle
    local coords = Config.VehicleSpawn
    QBCore.Functions.SpawnVehicle(vehicle, function(veh)
        SetVehicleNumberPlateText(veh, "phelpskitchenS"..tostring(math.random(1000, 9999)))
...
RegisterNetEvent('phelpskitchen:client:storeGarage', function()
...
RegisterNetEvent("phelpskitchen:client:frontTray1", function()
...
RegisterNetEvent("phelpskitchen:client:frontTray2", function()
...
RegisterNetEvent("phelpskitchen:client:frontTray3", function()
...
RegisterNetEvent("phelpskitchen:client:frontTray4", function()
...
RegisterNetEvent("phelpskitchen:client:jobHeater1", function()
...
RegisterNetEvent("phelpskitchen:client:jobFridge1", function()
...
RegisterNetEvent("phelpskitchen:client:ingredientStore", function()
    TriggerServerEvent("inventory:server:OpenInventory", "shop", "phelpskitchens", Config.Items)
end)

RegisterNetEvent('phelpskitchen:clientToggleDuty', function()
    TriggerServerEvent("QBCore:ToggleDuty")
end)

RegisterNetEvent("phelpskitchen:bill", function()
    ...
    if bill ~= nil then
        ...
        TriggerServerEvent("phelpskitchen:server:billPlayer", bill.citizenid, bill.billprice)
    end
end)